...
print("fitting beta model...")

bparam_names = ['a', 'b']

normed_data = (data - minval) / (maxval - minval)

def blikelihood(params):
    """Beta sample distribution"""
    a, b = params
    rv = scipy.stats.beta(a, b)
    probs = rv.pdf(normed_data).mean(axis=1)
    
    return np.log(np.clip(probs, 1e-300, 1e100)).sum()

def btransform(cube):
    """Gaussian sample distribution priors"""
    return cube * 10

bsampler = ultranest.ReactiveNestedSampler(
    bparam_names, blikelihood, btransform,
    log_dir=filename + '_out_beta', resume=True)
bresult = bsampler.run(frac_remain=0.5, viz_callback=viz_callback)
bsampler.print_results()


print("fitting gamma model...")

Gparam_names = ['k', 'scale']

def Glikelihood(params):
    """Gamma sample distribution"""
    k, scale = params
    rv = scipy.stats.gamma(k, scale=scale, loc=minval)
    probs = rv.pdf(data).mean(axis=1)
    
    return np.log(np.clip(probs, 1e-300, 1e100)).sum()

def Gtransform(cube):
    """Gamma sample distribution priors"""
    params = cube.copy()
    params[0] = cube[1] * 10
    params[1] = 3 * (maxval - minval) * cube[0]
    return params

Gsampler = ultranest.ReactiveNestedSampler(
    Gparam_names, Glikelihood, Gtransform,
    log_dir=filename + '_out_gamma', resume=True)
...
bband = PredictionBand(x)

for a, b in bresult['samples']:
    bband.add(scipy.stats.beta(a, b).pdf(xn) / (maxval - minval))
bband.line(color='b', label='Beta model')
bband.shade(alpha=0.5, color='b')
bband = PredictionBand(x)

Gband = PredictionBand(x)
for k, scale in Gresult['samples']:
    Gband.add(scipy.stats.gamma(a, scale=scale, loc=minval).pdf(x))
Gband.line(color='g', label='Gamma model')
Gband.shade(alpha=0.5, color='g')
